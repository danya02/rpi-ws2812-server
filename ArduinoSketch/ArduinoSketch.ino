    ...
    switch(headPos){
      ...
      case CHECKSUM:
        ...
        if(chk == (hi ^ lo ^ 0x55)) {
          ...
          outPos = 0;
          #ifdef PSU_PIN
            digitalWrite(PSU_PIN, (hi==0 && lo==0)?HIGH:LOW);
            //if(psuWasOn!=(hi==0 && lo==0)){for(byte i=0;i<10;i++){Serial.println("Waiting");delay(10);}}
            psuWasOn = (hi==0 && lo==0);
            Serial.println(psuWasOn);
          #endif
          Serial.print(hi);
          Serial.print(' ');
          Serial.println(lo);
